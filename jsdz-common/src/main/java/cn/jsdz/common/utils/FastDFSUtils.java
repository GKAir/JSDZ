...
public class FastDFSUtils {
	...
	// fdfsclient的配置文件的路径
	private static String CONF_NAME = "F:\\Demo\\jsdz-parent\\jsdz-common\\src\\main\\resources\\fdfs\\fdfs_client.conf";
	//private static String CONF_NAME = "/fdfs/fdfs_client.conf";
	static {
		try {
			// 配置文件必须指定全路径
			//String confName = FastDFSUtils.class.getResource(CONF_NAME).getPath();
           /* if (confName.contains("!")) {
                System.out.println(1);
                confName = confName.replace("!", "");
                confName = confName.replaceFirst("/", "");
                confName = confName.replace("file:","");
            }*/
            String confName = CONF_NAME;
            // 配置文件全路径中如果有中文，需要进行utf8转码
            System.out.println("confName1-->" + confName);
	...
	public static int deleteFile(String url) throws IOException, MyException {
        int result = 0;
        int index = url.indexOf("group");
        try {
            String groupName = url.substring(index, index + 6);
            String fileName = url.substring(index + 7);
            TrackerClient tracker = new TrackerClient();
            TrackerServer trackerServer = tracker.getConnection();
            StorageServer storageServer = null;
            StorageClient1 client = new StorageClient1(trackerServer, storageServer);
